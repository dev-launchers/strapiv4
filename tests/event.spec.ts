...
test.describe("/api/event", () => {
  test("Create new event on idea create", async ({ request }) => {
    ...
    const event = events.find(
      (item) => item.entityId === ideaId && item.entityType === "IdeaCard"
    );
  ...
  test("Create new event on idea update", async ({ request }) => {
    ...
    const event = events
      .reverse()
      .find(
        (item) => item.entityId === idea.id && item.entityType === "IdeaCard"
      );
  ...
  test("Create new event on idea stats update", async ({ request }) => {
    ...
    const event = events
      ...
      .find(
        (item) =>
          item.entityId === updatedIdea.id && item.entityType === "IdeaCard"
      );
  ...
  test("Create new event on comment create", async ({ request }) => {
    ...
    const event = events
      .reverse()
      .find(
        (item) => item.entityId === ideaId && item.entityType === "Comment"
      );
    ...
    expect(event.entityId).toBe(ideaId); // The entityId should be the ideaId related to the comment
